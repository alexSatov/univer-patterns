...
namespace Xrm.ReportUtility.Services
{
    public abstract class ReportServiceBase : IReportService
    {
        ...
        {
            return new ReportConfig // Создание конфигурации должно быть state билдером, т.к. хотим наличие обязательных ключей + избавляемся от копипаста
            {
        ...
        protected abstract DataRow[] GetDataRows(string text); // Фабричный метод, наследники определяют создание DataRow[]
    }
}
